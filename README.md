...
```bash
go install github.com/nobmurakita/cc-read-excel@latest
```

ビルド時にバージョンを埋め込む場合:

```bash
go build -ldflags "-X main.version=0.1.0" -o cc-read-excel .
```
...
```bash
mkdir -p ~/.claude/skills/cc-read-excel
curl -fsSL https://raw.githubusercontent.com/nobmurakita/cc-read-excel/main/SKILL.md -o ~/.claude/skills/cc-read-excel/SKILL.md
```
...
```bash
mkdir -p ~/.claude/skills/cc-read-excel
cp SKILL.md ~/.claude/skills/cc-read-excel/SKILL.md
```

インストール後、Claude Code が Excel ファイルの読み取りが必要な場面で自動的に cc-read-excel を使用する。
...
```bash
cc-read-excel scan --sheet 0 基本設計書.xlsx
```
...
```bash
cc-read-excel shapes --sheet 0 処理フロー.xlsx
```
...
```bash
cc-read-excel shapes --sheet 0 --extract-images /tmp/images 設計書.xlsx
```
...
```bash
cc-read-excel search --numeric ">100" 見積計算.xlsx
```
