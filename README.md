...
```bash
go install github.com/nobmurakita/cc-read-xlsx@latest
```

ビルド時にバージョンを埋め込む場合:

```bash
go build -ldflags "-X main.version=0.1.0" -o cc-read-xlsx .
```
...
```bash
mkdir -p ~/.claude/skills/cc-read-xlsx
curl -fsSL https://raw.githubusercontent.com/nobmurakita/cc-read-xlsx/main/SKILL.md -o ~/.claude/skills/cc-read-xlsx/SKILL.md
```
...
```bash
mkdir -p ~/.claude/skills/cc-read-xlsx
cp SKILL.md ~/.claude/skills/cc-read-xlsx/SKILL.md
```

インストール後、Claude Code が Excel ファイルの読み取りが必要な場面で自動的に cc-read-xlsx を使用する。
...
```bash
cc-read-xlsx scan --sheet 0 基本設計書.xlsx
```
...
```bash
cc-read-xlsx cells --sheet 0 --limit 5 見積計算.xlsx
```
...
```bash
cc-read-xlsx cells --sheet 0 --style --range "B3:K4" --limit 3 見積計算.xlsx
```
...
```bash
cc-read-xlsx shapes --sheet 0 処理フロー.xlsx
```
...
```bash
cc-read-xlsx search --text "マスタ" 運用シナリオ.xlsx
```
...
```bash
cc-read-xlsx search --numeric ">100" 見積計算.xlsx
```
