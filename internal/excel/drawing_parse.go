			...
			case "ext":
				...
				if inAnchor && len(groupStack) == 0 && anchorType != "two" {
					for _, attr := range t.Attr {
						...
						case "cx":
							anchorExtCX = safeAtoi(attr.Value)
						case "cy":
							anchorExtCY = safeAtoi(attr.Value)
						}
					}
				}
			...
			case "pos":
				...
				if inAnchor && anchorType == "abs" && len(groupStack) == 0 {
					for _, attr := range t.Attr {
						switch attr.Name.Local {
						case "x":
							anchorAbsX = safeAtoi(attr.Value)
						case "y":
							anchorAbsY = safeAtoi(attr.Value)
						}
					}
				}
...
func parseXfrm(t xml.StartElement) (rotation float64, flip string) {
	...
	for _, attr := range t.Attr {
		switch attr.Name.Local {
		case "rot":
			rotation = math.Round(float64(safeAtoi(attr.Value))/drawingMLRotUnit*100) / 100
		case "flipH":
			flipH = attr.Value == "1"
		case "flipV":
			flipV = attr.Value == "1"
