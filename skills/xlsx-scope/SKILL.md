...
cells, values, shapes, search は出力が大きくなるため、一時ファイルに保存して Read で分割して読む。

```bash
# 1. mktemp で一意なファイルを作り、コマンド出力を保存してパスを表示
f=$(mktemp /tmp/xlsx-scope-XXXXXX) && bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope cells --sheet 0 example.xlsx > "$f" && wc -l "$f"

# 2. 表示されたパスを使い Read で分割して読む（offset: 0始まり行番号, limit: 読む行数）
Read <表示されたパス> offset=0 limit=100
Read <表示されたパス> offset=100 limit=100

# 3. 読み終わったら削除
rm <表示されたパス>
```

info, scan など出力が小さいコマンドは stdout を直接読んでよい。
...
1. `image` サブコマンドで一時ファイルに保存する: `xlsx-scope image <file> <image_id>`
...
| コマンド | 説明 | stdout直接 | 詳細 |
|---------|------|-----------|------|
| `info <file>` | シート一覧・名前付き範囲 | OK | |
| `scan --sheet <s> <file>` | シート概要（used_range, value_count 等） | OK | |
| `cells <file>` | セルデータ（書式・結合セル含む） | 一時ファイル | [cells.md](references/cells.md) |
| `values <file>` | 値のみ行単位出力 | 一時ファイル | [values.md](references/values.md) |
| `shapes <file>` | 図形・画像 | 一時ファイル | [shapes.md](references/shapes.md) |
| `image <file> <image_id>` | 画像を一時ファイルに保存 | OK（パス出力） | |
| `search <file>` | セル値の検索 | 一時ファイル | [search.md](references/search.md) |
