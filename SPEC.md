...
**役割:** セルの値と書式を取得する。先頭に `meta` 行でレイアウト情報を出力し、続いてセルデータを出力する。`--range` で範囲を絞るか、`--range` なしで先頭からストリーミング取得する。内部的にワークシートXMLを自前でSAXパースし、`--limit` 到達時に即座に走査を打ち切る。
...
- バージョン文字列はビルド時に `-ldflags="-X github.com/nobmurakita/claude-xlsx-scope/internal/cmd.Version=<tag>"` で埋め込む。リリースビルド（GitHub Actions）では `GITHUB_REF_NAME`（タグ名）が設定される
