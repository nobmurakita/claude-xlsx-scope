...
典型フローは `info` → （必要なら `scan`）→ `cells` / `values` / `shapes` / `search` → `cleanup`。`scan` を経由せず `info` → `cells` でも可（`cells` の `meta` 行でレイアウト情報が取得できるため）。`scan` は大きいシートで used_range を事前に把握したい場合に有用。`shapes` は `scan` で `has_shapes: true` のシートに対して使用し、`image` は `shapes` の `image_id` を受けて実ファイルを抽出する。
...
- バージョン文字列はビルド時に埋め込む。未指定時のデフォルトは `latest`
...
`cells` / `search` で `--limit` に到達した場合、最終行に切り捨て通知を出力する。`--limit 0`（上限なし）の場合や、対象セル数が `--limit` 未満の場合は出力されない。`values` では `next_cell` の代わりに `next_row`（次に読むべき行番号）を返す。

```jsonl
{"truncated":true,"next_cell":"A51"}
```
...
- この行は `truncated` フィールドの有無で通常のセル行と区別できる
