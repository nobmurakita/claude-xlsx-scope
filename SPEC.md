...
サブコマンドは用途別に以下の3グループに分かれる。各コマンドの詳細は個別の節を参照。

- **探索**: `info`（ファイル概要） → `scan`（シート構造）
- **取得**: `cells`（セル値+書式） / `values`（値のみ軽量） / `shapes`（図形） / `image`（画像抽出） / `search`（検索）
- **後片付け/補助**: `cleanup`（一時ファイル削除） / `version`（バージョン表示）

典型フローは `info` → （必要なら `scan`）→ `cells` / `values` / `shapes` / `search` → `cleanup`。補足:

- `scan` を経由せず `info` → `cells` でも可（`cells` の `meta` 行でレイアウト情報が取得できるため）。`scan` は大きいシートで used_range を事前に把握したい場合に有用。
- `shapes` は `scan` で `shapes > 0` のシートに対して使用する。
- `image` は `shapes` の `image_id` を受けて実ファイルを抽出する。
...
```json
{"sheet":"機能一覧","used_range":"A1:H200","value_count":1520,"merged_cells":42,"style_variants":8,"shapes":11}
```
...
| `shapes` | int | 図形の総数（`shapes` コマンドの `shape_count` と一致） |

- `used_range` は dimension（XMLの `<dimension>` 要素）があればそのまま使用、なければセル走査で算出
- レイアウト情報（列幅、行高、デフォルトフォント等）は `cells` の `meta` 行で取得可能

### `xlsx-scope cells <file>`

**役割:** セルの値と書式を取得する。先頭に `meta` 行でレイアウト情報を出力し、続いてセルデータを出力する。`--range` で範囲を絞るか、`--range` なしで先頭から取得する。
...
| `--start <cell>` | 開始位置（例: `A51`）。行優先の走査順で指定セル以降のセルを出力する（指定セルの行の途中から開始し、次の行は走査範囲の左端列から走査する。`--range` 併用時は range の左端列、未指定時は A 列）。`--range` と併用可（範囲内でのページングに使用） | 先頭セル |
...
| `--start <row>` | 開始行番号（1始まり）。行単位での再開用。行途中からの再開はサポートしない（`cells --start` と異なる） | 先頭行 |
...
{"row":1,"values":["ID","名前","数量","日付"]}
{"row":2,"values":[1,"商品A",10,"2025-03-19"]}
{"row":4,"values":[2,"商品B",null,"2025-03-20"]}
{"truncated":true,"next_row":101}
```

- `meta` 行に `cols` 配列で列名マッピングを出力
- 各行は `{"row":N,"values":[...]}` 形式。`values` は `cols` と同じインデックスで整列する
- 値は `display`（表示文字列）があれば優先出力（日付・数値フォーマット済み）。値なしセルは `null`
- 空行はスキップ、末尾の null はトリム（途中の null は保持）
- `--limit 0` で上限なし
- `--limit` に到達した場合、最終行に `{"truncated":true,"next_row":101}` を出力する。`next_row` を `--start` に渡して続きを取得する
...
- `--limit 0` で上限なし
- 書式情報（塗りつぶし、枠線、フォント）は常に出力する
- 画像（`xdr:pic`）はメタデータのみ出力する。ファイル抽出は `image` サブコマンドで別途行う

**出力例:**（`meta` 行の詳細は後述）

```jsonl
{"meta":true,"shape_count":8,"connector_count":3}
{"id":1,"type":"flowChartTerminator","name":"開始","text":"開始","cell":"B2:D3","z":0,"fill":"#4A86E8","line":{"color":"#000000","style":"solid","width":1},"font":{"bold":true,"color":"#FFFFFF","size":11}}
...
| `pos` | object | ポイント座標。`{x, y, w, h}` で左上原点。アンカーの列幅・行高・セル内オフセット（EMU）から算出。グループ内の子要素では省略 |
...
| `from_idx` | number | 接続元の接続ポイントインデックス。`a:stCxn` の `idx` 属性。接続情報がない場合は省略 |
| `to_idx` | number | 接続先の接続ポイントインデックス。`a:endCxn` の `idx` 属性。接続情報がない場合は省略 |
...
| `start` | object | コネクタの始点座標 `{x, y}`（ポイント）。`pos` と `flip` から算出 |
| `end` | object | コネクタの終点座標 `{x, y}`（ポイント）。`pos` と `flip` から算出 |
...
| フィールド | 型 | 説明 |
|-----------|-----|------|
| `fill` | string | 塗りつぶし色（`#RRGGBB` 形式）。`a:spPr` の `a:solidFill` を優先し、無い場合は `a:style` の `a:fillRef`（`idx="0"` は除く）から解決。`a:spPr` に明示の `a:noFill`（塗りつぶしなし）がある場合は `fillRef` を使わず省略 |
| `line` | object | 枠線情報。`a:spPr` の `a:ln` を優先し、線色が無い場合は `a:style` の `a:lnRef`（`idx="0"` は除く）で補う。`a:ln` 内に明示の `a:noFill`（線なし）がある場合は `line` 自体を省略 |
...
- **画像**: 常に `"picture"`（`xdr:pic` 要素に対応）
- **VMLフォームコントロール**: `"checkbox"`, `"radio"`, `"drop"`, `"list"`, `"spin"`, `"scroll"`, `"button"`, `"gbox"`, `"label"` のいずれか（後述「VMLフォームコントロール」参照）
...
| VMLコメント吹き出し（`vmlDrawing.vml` のコメント用 shape） | コメント自体は `cells` / `comment` で取得するため視覚情報のみ |
| ActiveXコントロール（`xl/activeX/activeX*.xml`） | バイナリ形式（`activeX*.bin`）で複雑 |

#### VMLフォームコントロール

VML（`xl/drawings/vmlDrawing*.vml`）で定義されるレガシー形式のフォームコントロール（チェックボックス、オプションボタン、ドロップダウン等）は、DrawingML 図形に続けて出力する。Excel における描画順と合わせ、`z` は DrawingML トップレベル図形の最大値の次から VML 内 `z-index` 順で連番を割り当てる。

コントロールの種別（`type`）と追加フィールドは以下の通り。`cell` は `<controls><control>` 配下の `<anchor>` から DrawingML アンカーと同じロジックで算出する。`text` には `<v:textbox>` 内のラベル（全角・半角前後の空白は除去）を出力する。

| `type` | objectType | 追加フィールド |
|---|---|---|
| `checkbox` | `CheckBox` | `checked`, `linked_cell` |
| `radio` | `Radio` | `checked`, `linked_cell` |
| `drop` | `Drop` | `list_range`, `selected_index`, `drop_lines`, `linked_cell` |
| `list` | `List` | `list_range`, `selected_index`, `sel_type`, `linked_cell` |
| `spin` | `Spin` | `min`, `max`, `val`, `inc`, `linked_cell` |
| `scroll` | `Scroll` | `min`, `max`, `val`, `inc`, `page`, `linked_cell` |
| `button` | `Button` | `macro` |
| `gbox` | `GBox` | なし（ラベルのみ `text` に出力） |
| `label` | `Label` | なし（ラベルのみ `text` に出力） |

**フィールドの出典:**

| フィールド | 型 | 出典 |
|---|---|---|
| `checked` | bool | ctrlProp の `checked="Checked"` |
| `linked_cell` | string | ctrlProp の `fmlaLink`（例: `"$A$1"`） |
| `list_range` | string | ctrlProp の `fmlaRange`（例: `"$B$1:$B$5"`） |
| `selected_index` | number | ctrlProp の `sel`（1始まり、未選択は省略） |
| `drop_lines` | number | ctrlProp の `dropLines`（ドロップダウンの表示行数） |
| `sel_type` | string | ctrlProp の `selType` を小文字化（`"single"` / `"multi"` / `"extend"`） |
| `min`, `max`, `val`, `inc`, `page` | number | ctrlProp の同名属性 |
| `macro` | string | ctrlProp の `fmlaMacro` |
...
- `<file>` — 対象の .xlsx / .xlsm ファイル
- `<image_id>` — `shapes` の picture 要素が出力する `image_id`（例: `xl/media/image1.png`）

**出力:**

- `{"file":"..."}` 形式のJSON1行（抽出先のパス）
- 抽出先ファイルはプレフィックス `xlsx-scope-tmp-` で `os.TempDir()` 直下に生成される。拡張子は `image_id` のものを維持する
...
### `xlsx-scope cleanup <file> [file...]`

**役割:** xlsx-scope が他サブコマンドで生成した一時ファイルを削除する。`info` / `cells` / `values` / `scan` / `shapes` / `image` / `search` が返した `file` パスを読み終わったタイミングで呼び出し、ディスクに残った一時ファイルを後片付けする。

**引数:**

- 1つ以上のファイルパスを指定する（必須）
- 安全チェック（意図しないファイルの削除を防ぐため、以下の条件を**すべて満たさない**ファイルはエラーで即座に中断）:
  1. basename が `xlsx-scope-tmp-` プレフィックスで始まること
  2. 親ディレクトリが `os.TempDir()` 直下であること。他コマンドの一時ファイルは `os.CreateTemp("", ...)` で生成され `os.TempDir()` の値をそのまま前置したパスが返るため、絶対パス化した後の `filepath.Dir` との文字列比較で一致を確認する
- TempDir 配下でファイルが存在しない場合は無視する（冪等。`cleanup` を重複実行しても安全）
- エラー時は途中で中断し、それ以降の引数は処理しない（成功分だけ削除された部分状態になる可能性あり）
...
| フィールド | 型 | 説明 |
|-----------|-----|------|
| `deleted` | number | 実際に削除されたファイル数（存在しなかったファイルは含まれない） |

- 出力は stdout に1行のJSON
- 他コマンドと異なり、一時ファイルへの書き出しは行わない（データ出力がないため）
...
- バージョン文字列はビルド時に `-ldflags` で埋め込んだ値をそのまま出力する（例: `v1.2.3`）。`v` プレフィックスは実装側では付与しない
- 埋め込まれていない場合のデフォルトは `latest`
- 他コマンドと異なり、一時ファイルへの書き出しは行わない

## セルの出力構造

`cells` / `search` の出力形式、および `values` の一部で共通する構造を定義する。出力形式はJSONL固定（1行1JSONオブジェクト）。

### `cells` のメタ情報（`meta`）

cells の最初の行に出力される。シートのレイアウト基準値とセル座標の起点を含む。幅・高さの値はすべてポイント単位で、`shapes` コマンドの `pos` と同じ座標系。

```jsonl
{"meta":true,"origin":{"x":0,"y":0},"default_width":63.23,"default_height":20,"col_widths":{"B:D":183.75,"H":225}}
```

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `meta` | bool | 常に `true`。メタ情報行の識別用 |
| `default_width` | number | デフォルト列幅（ポイント） |
| `default_height` | number | デフォルト行高（ポイント） |
| `col_widths` | object | デフォルトと異なる列幅のマップ（キー: 列名または範囲、値: ポイント幅）。連続する同じ幅の列は `"B:D"` のように範囲表記でまとめられる。差分がない場合は省略 |
| `origin` | object | 出力の起点セルとそのポイント座標。`--range` / `--start` 指定時は先行セル分の累積座標を含む |

`origin` により、任意のセルのポイント座標を `default_width` / `col_widths` / `default_height` / `row` の `height` から累積計算で求められる。

### 行情報

行高や非表示など行レベルの情報がある場合、その行のセル出力の前に行情報を1行出力する。`row` フィールドの有無でセル行と区別できる。行高がデフォルトかつ非表示でない行は行情報を省略する。
...
|-----------|-----|------|
| `row` | number | 行番号 |
| `height` | number | 行高（ポイント。デフォルト行高と異なる場合のみ出力） |
...
| `type` | string | 通常は省略（JSONの値の型や `formula` フィールドの有無から推測可能なため）。例外として空セル（`--include-empty` 時）のみ `"empty"` を出力 |
...
あるスタイルが初めて登場したとき `style` 定義行が出力され、以降の同一スタイルのセルは `s` で参照番号のみを出力する（スタイルの重複排除）。`rich_text` はセル固有のため `style` には含まれず、セル行にインライン出力される。

**`style` 定義行のフィールド:**

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `style` | number | スタイル定義の識別番号 |
| `font` | object | フォント情報（デフォルトと異なる場合のみ） |
| `fill` | object | 背景色（デフォルトと異なる場合のみ） |
| `border` | object | 罫線（デフォルトと異なる場合のみ） |
| `alignment` | object | 配置（デフォルトと異なる場合のみ） |

**セルのスタイル関連フィールド:**

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `s` | number | `style` 定義行の参照番号 |
| `rich_text` | array | リッチテキストラン（セル内の一部が異なる書式を持つ場合。セル行にインライン出力） |
...
- VML（`vmlDrawing.vml`）はコメント用 shape については視覚情報のみのためパースしない（フォームコントロールの shape は `shapes` コマンドで取得できる）
...
`cells` / `search` / `values` で `--limit` に到達した場合、最終行に切り捨て通知を出力する。`--limit 0`（上限なし）の場合や、対象が `--limit` 未満の場合は出力されない。

```jsonl
{"truncated":true,"next_cell":"A51"}    # cells / search
{"truncated":true,"next_row":101}       # values
```

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `truncated` | bool | 常に `true` |
| `next_cell` | string | `cells` / `search` の次の開始位置。そのまま `--start` に渡して続きを取得する |
| `next_row` | number | `values` の次の開始行番号。そのまま `--start` に渡して続きを取得する |

- この行は `truncated` フィールドの有無で通常のデータ行と区別できる
- `search` で続きを取得する場合、同じフィルタ条件（`--text`, `--numeric`, `--type`）を再度指定する必要がある

### 出力例

```jsonl
{"meta":true,"origin":{"x":0,"y":0},"default_width":63.23,"default_height":20,"col_widths":{"B:H":120}}
{"style":1,"font":{"size":16,"bold":true},"alignment":{"horizontal":"center"}}
{"style":2,"fill":{"color":"#D9E2F3"}}
{"row":1,"height":30}
{"cell":"B1","value":"基本設計書","merge":"B1:H1","s":1}
{"row":2}
{"cell":"B2","value":"文書番号","merge":"B2:C2","s":2}
```
...
- シートにセルが一つもない場合 → `scan` は `used_range` を省略する。`cells` は `meta` 行のみ出力（セルなし）で正常終了する
- `search` で結果が0件の場合 → 本体は0行のJSONL（空）で正常終了する。stdout には通常どおり `{"file":"...","lines":0}` を出力（`--stdout` 時は何も出力しない）
...
- `info` / `scan` / `cells` / `values` / `shapes` / `search` の出力は一時ファイル（プレフィックス `xlsx-scope-tmp-`）に書き出され、stdout にはファイルパスと行数のJSON（`{"file":"...","lines":N}`）のみを出力する。`--stdout` フラグで標準出力に直接書き出すことも可能（デバッグ用）
- `image` は一時ファイルに画像を抽出し、stdout にパスのJSON（`{"file":"..."}`）を出力する。`--stdout` 指定時はパスをそのまま出力する
- `cleanup` / `version` は一時ファイルを生成せず、結果を直接 stdout に出力する（`--stdout` の影響を受けない）

### フォーマット

- 対応形式は .xlsx および .xlsm（.xls は非対応）
- 出力形式はJSONL（`info` / `scan` は1行のJSON、`cells` / `values` / `search` / `shapes` は複数行のJSONL）
- 出力は常にUTF-8エンコーディング
- セル値に含まれる制御文字（改行、タブ等）はJSON仕様に従いエスケープする（`\n`, `\t` 等）
- デフォルトで空セルをスキップする
- 書式フィールドはデフォルト値と異なる場合のみ出力する
- シートのデフォルトフォントは styles.xml の最初のフォント定義から取得し、各セルの `font` はデフォルトフォントとの差分のみ出力する
- 列幅は `cells` の `meta` 行で出力する（デフォルト値との差分のみ）。行高は `cells` の行情報（`row`）で行が変わるごとに出力する（デフォルト行高との差分のみ）
- 色は可能な限り `#RRGGBB` 形式（HEX RGB）で出力する。テーマカラーは theme1.xml から RGB に変換し、tint 値がある場合は HSL 色空間で明度を調整して適用する
- 図形の `fill`/`line` が `a:style` の `fillRef`/`lnRef` 経由（spPr に明示指定なし）の場合、テーマの `fmtScheme`（`fillStyleLst`/`lnStyleLst`）の色変換（`lumMod`/`lumOff`/`satMod`/`satOff`/`tint`/`shade`）を phClr に適用する。変換は HSL 色空間での近似。`fillStyleLst` のエントリがグラデーション（`gradFill`）の場合は中央ストップ（`pos="50000"` に最も近いストップ）を代表色とする単色で近似する
- 日付値はISO 8601形式で統一する
- 結合セルは左上セルのみ出力する
- `--limit` のデフォルトは1000
...
| メモリ使用量の上限 | 設けない。メモリ制限やファイルサイズ制限は設定しない |
