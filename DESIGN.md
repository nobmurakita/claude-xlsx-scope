...
2. **`scan`** — 対象シートの used_range を取得する（任意）。`cells` の `--range` を決定するために使う。図形の有無（`has_shapes`）も確認できる
3. **`cells`** — セルデータを取得する。先頭に `_meta` 行でレイアウト情報（列幅、デフォルト行高等）を出力し、続いてセルデータを出力する
4. **`shapes`** — 図形・フローチャートを取得する。図形間の接続関係を含めて構造を把握する（`scan` で `has_shapes: true` のシートに対して使用）
...
| `col_widths` | object | デフォルトと異なる列幅のマップ（キー: 列名、値: ピクセル幅）。差分がない場合は省略 |
...
- エラーメッセージは stderr に `cc-read-xlsx: <メッセージ>` の形式で出力する。stdout には常にJSONL（またはJSON）のみを出力する
...
```jsonl
{"id":10,"type":"picture","name":"図 1","cell":"B2:F8","pos":{"x":120,"y":80,"w":640,"h":480},"z":5,"alt_text":"システム構成図","image_path":"/tmp/cc-read-xlsx-images-xxx/image_abc.png"}
```
