...
2. **`scan`** — 対象シートの構造を把握する（デフォルトフォント、データ領域の分布）。`regions` の `range` と `non_empty_cells` から、`dump` の `--range` と `--limit` を決定する
3. **`dump`** — `scan` で得た領域ごとにデータを取得する。`non_empty_cells` が `--limit`（デフォルト1000）以下なら一度で取得でき、超える場合は `--limit` を調整するか切り捨て通知の `next_cell` / `next_range` で続きを取得する
...
**役割:** セルの値と書式を取得する。通常は `scan` で得た `regions` の `range` を `--range` に渡して領域単位で取得する。`non_empty_cells` が `--limit` を超える領域は、切り捨て通知の `next_cell` / `next_range` で分割取得する。
...
**役割:** 対象シートの構造を把握し、効率的な `dump` 戦略を立てる。`regions` の各領域を `dump --range` にそのまま渡せる。`non_empty_cells` が大きい領域は `--limit` の調整や複数回の取得が必要と判断できる。また `default_font` は `dump` 出力の `font` 差分を解釈するために必要。列幅・行高もここで出力するため、`dump` 出力のトークン消費を抑えられる。
...
|-----------|-----|------|
| `sheet` | string | シート名 |
| `used_range` | string | Excelが認識する使用範囲（実際のデータ分布とは乖離する場合がある。`regions` の方が実態に即している） |
...
| `row_heights` | object | デフォルトと異なる行高のマップ（キー: 行番号文字列、値: 高さ）。差分がない場合は省略 |
| `regions` | array | 値が密集している矩形領域の一覧（走査順） |
...
- `col_widths` / `row_heights` はデフォルト値と異なるもののみ出力する。全列・全行がデフォルトの場合はフィールド自体を省略する
- `col_widths` / `row_heights` の出力対象は使用範囲（`used_range`）内に限定する
...
- 数値比較（`--numeric`）: `number` 型のセルに対してのみ比較演算を行う。`date` 型のセルは対象外とする（日付をシリアル値で比較する用途は想定しない）
  - `">100"` — 100より大きい
  - `">=100"` — 100以上
  - `"<50"` — 50未満
...
- 複数指定した場合はAND条件で絞り込む
- `--numeric` 単独指定時は数値セルのみが対象、`--type` 単独指定時は該当型のセルをすべて出力
- `--numeric` の値はシェルの解釈を避けるためクォートが必要（例: `--numeric ">100"`, `--numeric "100:200"`）

## セルの出力構造

出力形式はJSONL（1行1セル）固定。大きなファイルでも行単位でフィルタ・パイプ処理が可能。
...
| `type` | string | 値の型（`string`, `number`, `bool`, `date`, `formula`, `error`, `empty`）。`empty` は `--include-empty` 時のみ出力される |
| `merge` | string | 結合範囲（結合セルの場合のみ。例: `B3:D3`） |
| `formula` | string | 数式文字列（数式セルの場合のみ。例: `=SUM(D3:D9)`） |
| `link` | object | ハイパーリンク情報（リンクが設定されている場合のみ） |
| `hidden_row` | bool | 行が非表示の場合のみ `true` を出力 |
...
`dump` / `search` で `--limit` により出力が打ち切られた場合、最終行に切り捨て情報を出力する。両コマンドとも同じ形式で出力する。
...
- 列幅・行高は `scan` で出力する（デフォルト値との差分のみ）。`dump` / `search` のセル出力には含めない（列幅・行高は列/行単位の属性であり、セルごとに繰り返し出力するとトークンの無駄になるため）
