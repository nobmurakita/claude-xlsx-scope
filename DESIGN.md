...
- `--limit 0` で上限なし
- `--limit` に到達した場合、最終行に `{"_truncated":true,"next_row":101}` を出力する。`next_row` を `--start` に渡して続きを取得する
...
`origin` により、任意のセルのポイント座標を `default_width` / `col_widths` / `default_height` / `_row` の `height` から累積計算で求められる。
...
あるスタイルが初めて登場したとき `_style` 定義行が出力され、以降の同一スタイルのセルは `s` で参照番号のみを出力する（スタイルの重複排除）。`rich_text` はセル固有のため `_style` には含まれず、セル行にインライン出力される。

**`_style` 定義行のフィールド:**

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `_style` | number | スタイル定義の識別番号 |
...
```jsonl
{"_meta":true,"default_width":63.23,"default_height":20,"col_widths":{"B:H":120},"origin":{"x":0,"y":0}}
{"_style":1,"font":{"size":16,"bold":true},"alignment":{"horizontal":"center"}}
{"_style":2,"fill":{"color":"#D9E2F3"}}
{"_row":1,"height":30}
