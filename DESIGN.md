...
- エラーメッセージは stderr に `read-xlsx: <メッセージ>` の形式で出力する。stdout には常にJSONL（またはJSON）のみを出力する
...
```jsonl
{"id":10,"type":"picture","name":"図 1","cell":"B2:F8","pos":{"x":120,"y":80,"w":640,"h":480},"z":5,"alt_text":"システム構成図","image_path":"/tmp/read-xlsx-images-xxx/image_abc.png"}
```
