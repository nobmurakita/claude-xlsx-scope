...
2. **`scan`** — 対象シートの used_range を取得する（任意）。`dump` の `--range` を決定するために使う。図形の有無（`has_drawings`）も確認できる
...
5. **`search`** — 特定の値やセル型を検索する。シート全体から条件に合うセルだけを抽出したい場合に使用する。`dump` で全体を取得するより効率的
