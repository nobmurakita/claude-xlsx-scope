# exceldump 設計ドキュメント

Excel ファイル（.xlsx / .xlsm）の内容をCLIからダンプするGoツール。
Claude CodeがExcel資料（通常の表、Excel方眼紙の仕様書など）を読み取る用途を主眼とする。

## コマンド構成と利用フロー

Claude Code からの典型的な利用フローは以下の通り:

1. **`info`** — ファイルの全体像を把握する（シート一覧、定義名）。どのシートを読むべきか判断する
2. **`scan`** — 対象シートの構造を把握する（デフォルトフォント、データ領域の分布）。dimension がある場合は `regions` の `range` と `non_empty_cells` から、`dump` の `--range` と `--limit` を決定できる
3. **`dump`** — セルデータを取得する。`scan` で得た領域ごとに `--range` で取得するか、`--range` なしで先頭からストリーミング取得する。行が変わるごとに行情報（行高、非表示）も出力する
4. **`search`** — 特定の値やセル型を検索する。シート全体から条件に合うセルだけを抽出したい場合に使用する。`dump` で全体を取得するより効率的

`scan` を経由せず `info` → `dump` で直接データを取得することも可能。特に dimension がないファイル（Google Sheets 由来等）では `scan` の regions が利用できないため、`dump` を直接実行する方が効率的

### `exceldump info <file>`

**役割:** ファイルレベルの概要を把握する。シート一覧から対象シートを特定し、以降の `scan` / `dump` / `search` に渡す `--sheet` を決定する。

ファイルレベルの概要をJSON形式で出力する。

- シート一覧（名前、インデックス、種類、表示状態）
- 名前付き範囲（定義名）一覧（名前、スコープ、参照先）。`refer` はexcelizeが返す文字列をそのまま出力する（削除済みシートへの参照や数式を含む定義名もそのまま）

**出力例:**

```json
{
  "file": "基本設計書.xlsx",
  "defined_names": [
    {"name": "マスタ", "scope": "Workbook", "refer": "Sheet1!$A$1:$D$100"},
    {"name": "入力範囲", "scope": "Sheet2", "refer": "Sheet2!$B$3:$F$20"}
  ],
  "sheets": [
    {"index": 0, "name": "表紙", "type": "worksheet"},
    {"index": 1, "name": "機能一覧", "type": "worksheet"},
    {"index": 2, "name": "非表示データ", "type": "worksheet", "hidden": true},
    {"index": 3, "name": "グラフ", "type": "chartsheet"}
  ]
}
```

**`sheets` 配列の各要素:**

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `index` | number | シートの0始まりインデックス |
| `name` | string | シート名 |
| `type` | string | シート種類（`worksheet`, `chartsheet` 等。excelizeが返す値をそのまま出力する） |
| `hidden` | bool | 非表示の場合のみ `true` を出力（`hidden` / `veryHidden` を区別しない）。表示状態のシートでは省略 |

### `exceldump dump <file>`

**役割:** セルの値と書式を取得する。`scan` で得た `regions` の `range` を `--range` に渡して領域単位で取得するか、`--range` なしで先頭からストリーミング取得する。内部的に excelize の `Rows()` イテレータでストリーミング処理を行い、`--limit` 到達時に即座に走査を打ち切る。

指定シートのセル情報を丸ごとダンプする。

**オプション:**

| オプション | 説明 | デフォルト |
|-----------|------|-----------|
| `--sheet <name\|index>` | 対象シート（名前 or 0始まりインデックス） | 最初のシート |
| `--range <range>` | セル範囲。Excel記法で指定（例: `A1:H20`, `A:F`, `1:20`） | シートの使用範囲全体 |
| `--start <cell>` | 開始位置（例: `A51`）。行優先の走査順で指定セル以降のセルを出力する（指定セルの行の途中から開始し、次の行はA列から走査する）。`--range` との併用不可 | 先頭セル |
| `--include-empty` | 空セルも出力する | OFF（空セルはスキップ） |
| `--no-style` | 書式情報を省略（値と座標のみ） | OFF |
| `--limit <n>` | 出力セル数の上限 | 1000 |

- `--limit 0` で上限なし
- `--include-empty` 指定時は空セルも出力対象に含まれるため、対象セル数が大幅に増える。必要に応じて `--range` で範囲を絞るか `--limit` を調整すること

### `exceldump scan <file>`

**役割:** 対象シートの構造を把握し、効率的な `dump` 戦略を立てる。`default_font` は `dump` 出力の `font` 差分を解釈するために必要。dimension がある場合は `regions` の各領域を `dump --range` にそのまま渡せる。

指定シートの詳細情報と非空セルの分布を出力する。

**dimension の有無による出力の違い:**

dimension（シートXMLの `<dimension>` 要素）がある場合は全行を走査し、`used_range`、`regions`、`row_heights` を含む完全な情報を返す。dimension がない場合（Google Sheets 由来等）は全行走査を行わず、シートメタ情報と `col_widths` のみの軽量出力を返す。行高は `dump` の行情報（`_row`）から取得できるため、解析精度に差はない。

**オプション:**

| オプション | 説明 | デフォルト |
|-----------|------|-----------|
| `--sheet <name\|index>` | 対象シート（名前 or 0始まりインデックス） | 最初のシート |

**領域分割ロジック:**

1. シート内の非空セルの座標を全取得する
2. 使用されている行番号を集め、3行以上の連続する空行で行バンドに分割する
3. 使用されている列番号を集め、3列以上の連続する空列で列バンドに分割する
4. 行バンド × 列バンドの直積で矩形候補を生成する
5. 各矩形内の非空セル数をカウントし、0件の矩形は除外する

**出力例:**

```json
{
  "sheet": "機能一覧",
  "used_range": "A1:H200",
  "tab_color": "#FF0000",
  "default_font": {"name": "游ゴシック", "size": 11},
  "default_width": 8.43,
  "default_height": 15.0,
  "col_widths": {"B": 24.5, "C": 24.5, "H": 30.0},
  "row_heights": {"1": 30.0, "2": 18.0},
  "regions": [
    {"range": "A1:H5", "non_empty_cells": 30},
    {"range": "A20:H80", "non_empty_cells": 350},
    {"range": "A150:D160", "non_empty_cells": 40}
  ]
}
```

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `sheet` | string | シート名 |
| `used_range` | string | シートの使用範囲。dimension がない場合は省略 |
| `tab_color` | string | タブ色（設定されている場合のみ） |
| `default_font` | object | シートのデフォルトフォント（後述の検出ロジックによる） |
| `default_width` | number | シートのデフォルト列幅（Excel内部単位: 標準フォントの文字幅基準） |
| `default_height` | number | シートのデフォルト行高（ポイント単位） |
| `col_widths` | object | デフォルトと異なる列幅のマップ（キー: 列名、値: 幅）。差分がない場合は省略 |
| `row_heights` | object | デフォルトと異なる行高のマップ（キー: 行番号文字列、値: 高さ）。dimension がない場合は省略（`dump` の行情報から取得可能） |
| `regions` | array | 値が密集している矩形領域の一覧（走査順）。dimension がない場合は省略 |
| `regions[].range` | string | 矩形範囲（そのまま `dump --range` に渡せる） |
| `regions[].non_empty_cells` | number | 当該矩形内の非空セル数 |

- `used_range`、`regions`、`row_heights` は dimension がある場合のみ出力する。dimension がない場合（Google Sheets 由来等）は全行走査のコストを回避するため省略する
- 領域が1つの場合（分割不要）でも `regions` 配列に1要素として出力する
- `regions` の順序は走査順（行優先: 上→下、同一行バンド内は左→右）
- 分割閾値は固定（3行/3列）。オプションによる変更は不可
- `tab_color` はタブ色が設定されている場合のみ出力
- `default_font` の検出ロジックは「設計方針」セクションを参照
- `col_widths` はデフォルト値と異なるもののみ出力する。dimension がある場合は `used_range` 内に限定、ない場合はシートの列定義から取得する
- `row_heights` はデフォルト値と異なるもののみ出力する。全行がデフォルトの場合はフィールド自体を省略する

### `exceldump search <file>`

**役割:** 特定の値やセル型をシート内から検索する。シート全体を `dump` して手元でフィルタするより効率的。例えば「特定のキーワードを含むセルの位置を特定 → その周辺を `dump --range` で取得」という使い方ができる。

セル値を検索し、一致するセルの情報を `dump` と同じ出力形式で出力する。

**オプション:**

| オプション | 説明 | デフォルト |
|-----------|------|-----------|
| `--query <text>` | 検索文字列（部分一致） | なし |
| `--numeric <expr>` | 数値比較（例: `">100"`, `"100:200"`, `"=42"`） | なし |
| `--type <type>` | セルの型でフィルタ（`string`, `number`, `date`, `bool`, `formula`） | すべて |
| `--sheet <name\|index>` | 対象シート（名前 or 0始まりインデックス） | 最初のシート |
| `--range <range>` | セル範囲。Excel記法で指定（例: `A1:H20`） | シートの使用範囲全体 |
| `--start <cell>` | 開始位置（例: `A51`）。`dump` の `--start` と同様の走査順で指定セル以降を検索する。`--range` との併用不可 | 先頭セル |
| `--no-style` | 書式情報を省略（値と座標のみ） | OFF |
| `--limit <n>` | 出力セル数の上限 | 1000 |

- `--limit 0` で上限なし
- `--query`, `--numeric`, `--type` のうち少なくとも1つの指定が必須

**検索の挙動:**

- 文字列検索（`--query`）: `display`（表示文字列）と `value` の両方に対して大文字・小文字を区別しない部分一致検索を行う（いずれかにマッチすればヒット）。全角・半角は区別する。正規表現には対応しない。数値・日付・真偽値セルの `value` は出力時と同じ文字列表現に変換してから比較する（例: 数値 `1000` は `"1000"` として、日付は ISO 8601 形式として比較）
- 例: `--query "令和"` → 和暦表示のセルにヒット、`--query "2025-03"` → ISO 8601形式の `value` にヒット
- 数値比較（`--numeric`）: `number` 型のセルに対して比較演算を行う。`formula` 型のセルもキャッシュ値が数値の場合は対象とする。`date` 型のセルは対象外とする（日付をシリアル値で比較する用途は想定しない）
  - `">100"` — 100より大きい
  - `">=100"` — 100以上
  - `"<50"` — 50未満
  - `"100:200"` — 100以上200以下（範囲指定）
  - `"=42"` — 42と等しい（浮動小数点の誤差を考慮し、差の絶対値が 1e-9 以下なら等しいとみなす）
- 複数指定した場合はAND条件で絞り込む
- `--type` フィルタは `formula` 型セルのキャッシュ値の型も考慮する（例: `--type number` は数値キャッシュの数式セルにもヒットする）
- `--numeric` 単独指定時は数値セル（数式の数値キャッシュ含む）のみが対象、`--type` 単独指定時は該当型のセルをすべて出力
- `--numeric` の値はシェルの解釈を避けるためクォートが必要（例: `--numeric ">100"`, `--numeric "100:200"`）

## セルの出力構造

出力形式はJSONL固定。大きなファイルでも行単位でフィルタ・パイプ処理が可能。

### 行情報

行が変わるタイミングで、セル行の前に行情報を1行出力する。`_row` フィールドの有無でセル行と区別できる。

```jsonl
{"_row":1,"height":30}
{"cell":"A1","value":"タイトル"}
{"cell":"B1","value":"内容"}
{"_row":2}
{"cell":"A2","value":"データ"}
```

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `_row` | number | 行番号 |
| `height` | number | 行高（デフォルト行高と異なる場合のみ出力） |
| `hidden` | bool | 行が非表示の場合のみ `true` を出力 |

### セルフィールド

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `cell` | string | セル座標（例: `B3`） |
| `value` | any | セルの値（文字列、数値、真偽値） |
| `display` | string | Excelでの表示文字列（`value` を文字列化した結果と異なる場合のみ出力。後述の詳細を参照） |
| `type` | string | `date` または `error` の場合のみ出力。他の型はJSONの値の型や `formula` フィールドの有無から推測可能なため省略する |
| `merge` | string | 結合範囲（結合セルの場合のみ。例: `B3:D3`） |
| `formula` | string | 数式文字列（数式セルの場合のみ。例: `=SUM(D3:D9)`） |
| `link` | object | ハイパーリンク情報（リンクが設定されている場合のみ） |
| `hidden_col` | bool | 列が非表示の場合のみ `true` を出力 |

**型の判定ルール:**

- `type` 省略 + `value` が文字列 → string
- `type` 省略 + `value` が数値 → number
- `type` 省略 + `value` が true/false → bool
- `type` 省略 + `formula` あり → formula（`value` はキャッシュ値）
- `type` 省略 + `value` なし → empty（`--include-empty` 時のみ出現）
- `type: "date"` → 日付（`value` は ISO 8601 文字列）
- `type: "error"` → エラー値（`value` は `#N/A` 等の文字列）

### 日付値

日付セルの `value` はISO 8601形式で出力する。

- 日付のみ: `"2025-03-15"`
- 日時: `"2025-03-15T10:30:00"`
- 時刻のみ: `"10:30:00"`

日付の判定はexcelizeの数値フォーマット情報に基づく。Excelは日付を内部的に数値（シリアル値）として保持しており、セルの表示形式が日付系フォーマット（`yyyy/mm/dd` 等）であれば日付セルとして扱う。excelizeが認識できないカスタム表示形式の場合は数値セル（`number`）として出力する。

### `display` フィールド

`display` はセルがExcel上で表示する文字列をそのまま出力する。`value` をJSONエンコードした結果と同一の場合は省略する（例: 数値 `1000` → JSON表現 `"1000"` と比較。Go の `encoding/json` が生成する数値表現が基準）。bool値の場合、`value` は JSON の `true`/`false` だが Excel の表示は `TRUE`/`FALSE` であるため `display` を出力する。

**対象:**

- 数値フォーマット: 通貨（`¥1,000`）、パーセント（`50%`）、日付（`2025年3月15日`）、カスタム書式等
- 数式セル: 計算結果の表示文字列（例: `value` が `150` でも表示形式が通貨なら `display` は `"¥150"`）
- ハイパーリンク: セルの表示テキスト（リンク先URLとは異なる表示文字列がある場合）

**制限事項:**

- 条件付き書式（セル値に応じた動的なフォーマット）には非対応。セル自体の表示形式のみ適用する
- ロケール依存のフォーマット（`[$-ja-JP]` 等）は、excelizeがサポートする範囲でのみ再現される
- excelizeが解釈できないカスタム表示形式の場合、`display` は省略される（`value` のみ出力）

### 空セル（`--include-empty` 時）

```jsonl
{"cell":"C3","type":"empty"}
```

- `value` は省略する（`null` や空文字は出力しない）
- 書式フィールドはデフォルトと異なる場合のみ出力する（`--no-style` でない場合）

### 結合セル

- 結合領域の左上セルのみを出力する。結合に含まれる他のセルは出力しない（`--include-empty` 時も同様にスキップする）
- `--range` で結合領域の一部のみが含まれる場合、左上セルが範囲内なら出力する（左上セルが範囲外なら出力しない）

### セルの走査順序

セルは行優先順（row-major order）で走査する: A1 → B1 → C1 → ... → A2 → B2 → ... 。`--limit` による切り捨てや `next_cell` はこの走査順に基づく。

### `--range` の指定形式

以下の形式をサポートする:

- 矩形範囲: `A1:H20`
- 列のみ: `A:F`（行はExcelの使用範囲（`scan` の `used_range`）で補完）
- 行のみ: `1:20`（列はExcelの使用範囲（`scan` の `used_range`）で補完）
- 単一セル: `B5`（`B5:B5` と同等）
- 単一行: `3:3`
- 単一列: `A:A`

列のみ・行のみ指定では `used_range` から省略された行・列の上下限を補完する。空シート（`used_range` が空）の場合は空出力で正常終了する。

### 書式フィールド（`--no-style` で省略可能）

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `font` | object | フォント情報 |
| `fill` | object | 背景色 |
| `border` | object | 罫線 |
| `alignment` | object | 配置 |
| `rich_text` | array | リッチテキストラン（セル内の一部が異なる書式を持つ場合） |

### リッチテキスト

セル内の文字列の一部が異なる書式を持つ場合、`value` にはプレーンテキストを格納し、`rich_text` に書式付きランの配列を格納する。

```jsonl
{"cell":"B2","value":"本機能は必須とする。","rich_text":[{"text":"本機能は"},{"text":"必須","font":{"bold":true,"color":"#FF0000","size":14}},{"text":"とする。"}]}
```

- `value` は常にプレーンテキスト（検索・概要把握用）
- `rich_text` はセル内に書式の異なるランが存在する場合のみ出力
- 各ランの `font` はセルレベルの `font` オブジェクトと同じ構造。デフォルト値のフィールドは省略する。書式がセルのフォントと同一のランでは `font` 自体を省略する
- `--no-style` 指定時は `rich_text` を省略

### 数式

数式セルでは `type` を `formula` とし、`formula` フィールドに数式文字列を格納する。`value` にはExcelが最後に保存した計算結果（キャッシュ値）が入る。

```jsonl
{"cell":"D10","value":150,"display":"¥150","type":"formula","formula":"=SUM(D3:D9)"}
```

- 数式の再計算は行わない（Excelエンジンではないため）
- キャッシュ値が存在しない場合、`value` は `null`
- 共有数式（shared formula）: excelizeが展開した数式文字列を格納する。excelizeが展開できない場合は `formula` を空文字 `""` とし `type` は `formula` のまま出力する（`value` はキャッシュ値を出力する）
- 配列数式（CSE数式）: 波括弧付きで格納する（例: `"{=SUM(A1:A10*B1:B10)}"`）。動的配列（スピル）の場合、スピル元セルのみに `formula` を格納し、スピル先セルは通常の値セルとして出力する

### エラー値

セルがExcelのエラー値を含む場合、`type` を `error` とし、`value` にエラー文字列をそのまま格納する。

```jsonl
{"cell":"A1","value":"#N/A","type":"error"}
{"cell":"B2","value":"#REF!","type":"error","formula":"=Sheet1!A1"}
```

- エラー値: `#N/A`, `#REF!`, `#VALUE!`, `#DIV/0!`, `#NAME?`, `#NULL!`, `#NUM!`
- 数式セルの計算結果がエラーの場合、`type` は `error` とし `formula` も出力する

### ハイパーリンク

セルにハイパーリンクが設定されている場合、`link` オブジェクトを出力する。

```jsonl
{"cell":"C5","value":"参照先ドキュメント","type":"string","link":{"url":"https://example.com/doc.pdf"}}
{"cell":"A1","value":"詳細はSheet2参照","type":"string","link":{"location":"Sheet2!A1"}}
```

- 外部URL → `url` フィールド
- シート内・ブック内リンク → `location` フィールド
- 両方が設定されている場合は両方出力

### font オブジェクト

```json
{"name": "MS ゴシック", "size": 11, "bold": true, "italic": true, "strikethrough": true, "underline": "single", "color": "#FF0000"}
```

- セルのフォントがシートの `default_font`（`scan` で出力される値）と完全に一致する場合、`font` フィールド自体を省略する
- `default_font` と異なるフィールドのみ出力する（例: `default_font` と name/size が同じで bold だけ異なる場合 → `{"bold": true}` のみ）
- 値がデフォルト（bold: false 等）のフィールドも省略する
- `italic`, `strikethrough` は bool 型。`false` の場合は省略する
- `underline` は文字列型で、値は `single`, `double`, `singleAccounting`, `doubleAccounting` のいずれか（下線なしの場合は省略）

### fill オブジェクト

ソリッド塗りつぶし（単色）の前景色のみ出力する。パターン塗りつぶしやグラデーションには対応しない。

```json
{"color": "#D9E2F3"}
```

### border オブジェクト

```json
{"top": {"style": "thin"}, "bottom": {"style": "thin", "color": "#FF0000"}, "left": {"style": "medium"}, "right": {"style": "thin"}, "diagonal_up": {"style": "thin"}}
```

- 罫線がないエッジは省略する。各エッジは `style` と `color` を持つオブジェクト
- エッジ: `top`, `bottom`, `left`, `right`, `diagonal_up`（左下→右上）, `diagonal_down`（左上→右下）
- `color` はデフォルト（黒）の場合は省略する
- スタイル値: `thin`, `medium`, `thick`, `dashed`, `dotted`, `double`, `hair`, `mediumDashed`, `dashDot`, `mediumDashDot`, `dashDotDot`, `mediumDashDotDot`, `slantDashDot`

### alignment オブジェクト

```json
{"horizontal": "center", "vertical": "center", "wrap": true, "indent": 2, "text_rotation": 90, "shrink_to_fit": true}
```

- `horizontal`: `left`, `center`, `right`, `fill`, `justify`, `distributed`。デフォルト（`general`）の場合は省略
- `vertical`: `top`, `center`, `bottom`, `justify`, `distributed`。デフォルト（`bottom`）の場合は省略
- `wrap`: bool。テキスト折り返し。`false` の場合は省略
- `indent`: number。インデントレベル。0の場合は省略
- `text_rotation`: number。テキストの回転角度（-90〜90、または255で縦書き）。0の場合は省略
- `shrink_to_fit`: bool。縮小して全体を表示。`false` の場合は省略

### 切り捨て通知

`dump` / `search` はストリーミング処理のため、`--limit` 到達時に即座に走査を打ち切る。ストリーミングでは打ち切り後の残りセル数が不明なため、切り捨て通知は出力しない。続きを取得するには、最後に出力されたセルの次の位置を `--start` に指定する。

**注意:** 以下の切り捨て通知形式は、将来的に全体数の算出が可能な場合に使用する予定の形式である。現在のストリーミング実装では出力されない。

```jsonl
{"_truncated":true,"total":2000,"output":1000,"next_cell":"A51"}
```

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `_truncated` | bool | 常に `true` |
| `total` | number | 対象セルの総数。`dump` では指定範囲内の非空セル数（`--include-empty` 時は全セル数）。`search` では指定範囲内かつフィルタ条件に合致するセルの総数 |
| `output` | number | 出力したセル数 |
| `next_cell` | string | 打ち切られた次の開始位置。そのまま `--start` に渡して続きを取得する |
| `next_range` | string | `--range` 指定時のみ出力。続きを取得するための `--range` 値（例: `A51:H100`） |

- この行は `_truncated` フィールドの有無で通常のセル行と区別できる
- `--limit 0`（上限なし）の場合、この行は出力されない
- `--range` 指定時: `next_range` をそのまま `--range` に渡して続きを取得する
- `--range` 未指定時: `next_cell` を `--start` に渡して続きを取得する
- `next_range` は元の `--range` の終端を保持する。`--range` が列のみ指定（`A:F`）や行のみ指定（`1:20`）の場合、Excelの使用範囲から省略された行・列の上下限を補完して算出する（例: `A:F` で使用範囲が `A1:F100` なら終端行を100として扱う）
- `search` で続きを取得する場合、同じフィルタ条件（`--query`, `--numeric`, `--type`）を再度指定する必要がある

### 出力例

```jsonl
{"_row":1,"height":30}
{"cell":"B1","value":"基本設計書","merge":"B1:H1","font":{"size":16,"bold":true},"alignment":{"horizontal":"center"}}
{"_row":2}
{"cell":"B2","value":"文書番号","merge":"B2:C2","fill":{"color":"#D9E2F3"}}
```

## 技術選定

- **言語:** Go
- **Excelパーサー:** [excelize](https://github.com/qax-os/excelize) v2
- **CLIフレームワーク:** [cobra](https://github.com/spf13/cobra)

## エラーハンドリング

- ファイルが存在しない / 読み取れない → エラーメッセージを stderr に出力
- .xlsx / .xlsm 以外のファイル → 「.xlsx / .xlsm 形式のみ対応」のエラーメッセージ
- 存在しないシート名/インデックス → 利用可能なシート一覧をエラーメッセージに含める
- 不正なオプション値（range, numeric） → パース失敗箇所を示すエラーメッセージ。論理的に不正な範囲（例: 終端が始端より前）もエラーとする
- `--range` と `--start` の併用 → エラーメッセージを出力（排他オプション）
- パスワード保護されたファイル → 非対応としてエラーメッセージを出力
- 破損したファイル（不正なzip構造等） → excelizeのエラーメッセージをそのまま出力
- ワークシート以外のシート（チャートシート等）を `scan` / `dump` / `search` で指定 → 「ワークシートのみ対応」のエラーメッセージ
- シートにセルが一つもない場合 → `scan` は `used_range` を空文字、`regions` を空配列で出力する。`default_font`、`default_width`、`default_height` は空シートでも出力する（シートのメタ情報であり、セルの有無に依存しない）。`col_widths` / `row_heights` は使用範囲がないため省略する。`dump` は空出力（0行のJSONL）で正常終了する
- `search` で結果が0件の場合 → 終了コード 1（`grep` 互換。パイプ連携で利用可能）。`--limit` で打ち切られた場合（結果が1件以上ある場合）は終了コード 0
- 終了コード: 0=成功、1=検索結果なし（`search` コマンドのみ）、2=エラー。上記のエラーケースはすべて終了コード 2
- エラーメッセージは stderr に `exceldump: <メッセージ>` の形式で出力する。stdout には常にJSONL（またはJSON）のみを出力する

## 設計方針

- 対応形式は .xlsx および .xlsm（.xls は非対応）
- excelizeの通常読み取り（`OpenFile`）を使用する
- `dump` / `search` の出力形式はJSONL固定（1行1セルのストリーム出力）。`info` / `scan` はJSON形式で出力する
- デフォルトで空セルをスキップする（Excel方眼紙は空セルが大量にあるため）
- 書式フィールドはデフォルト値と異なる場合のみ出力し、出力サイズを抑える
- シートのデフォルトフォントは列スタイルから検出する。使用範囲（`used_range`）内の列を対象に、列スタイルに設定されたフォントの出現数をカウントし、最も多いフォントを採用する。同数の場合は列インデックスが最も小さい列のフォントを採用する。列スタイルが未設定の場合はブックのデフォルトフォントをフォールバックとする
- `dump` / `search` の各セルの `font` は `scan` で出力される `default_font` との差分のみ出力する（トークン効率のため）
- 列幅は `scan` で出力する（デフォルト値との差分のみ）。行高は `dump` の行情報（`_row`）で行が変わるごとに出力する（デフォルト行高との差分のみ）。これにより `scan` での全行走査を回避しつつ、`dump` のストリーミング出力で行高を取得できる
- 色は可能な限り `#RRGGBB` 形式（HEX RGB）で出力する。テーマカラーはexcelizeの `GetThemeColor` でRGBに変換し、tint値がある場合はHSL色空間で明度を調整して適用する。RGB に変換できない場合（テーマ情報の欠損等）は、excelize から取得できる生の値（テーマインデックスや indexed color 番号等）をそのまま出力する。色情報がある限り省略しない
- 日付値はISO 8601形式で統一する
- 結合セルは左上セルのみ出力する
- `--limit` のデフォルトは1000（Claude Codeのコンテキスト窓を考慮）
- 出力は常にUTF-8エンコーディング
- セル値に含まれる制御文字（改行、タブ等）はJSON仕様に従いエスケープする（`\n`, `\t` 等）
- SKILL化を見据え、コマンドの出力はパイプ処理しやすい形式にする

## コマンド構成（その他）

### `exceldump version`

バージョン情報をプレーンテキストで出力する。

```
exceldump version 0.1.0
```

バージョン番号は `go build -ldflags` でビルド時に埋め込む。未設定の場合は `dev` を表示する。

## 対応しない機能

以下の機能は意図的に対応しない。理由とともに記録する。

| 機能 | 理由 |
|------|------|
| ストリーミングモード（`--streaming`） | excelizeの `Rows()` による行単位読み取りは日付判定不可・書式取得不可など制約が多く、実装コストに見合わない。24GB程度のメモリ環境では通常モードで十分対応可能 |
| .xls 形式の読み取り | excelizeが非対応。旧形式のファイルは事前に .xlsx へ変換して使用する |
| 数式の再計算 | Excelエンジンではないため再計算は行わない。キャッシュ値（最後に保存された計算結果）を出力する |
| パスワード保護されたファイル | excelizeの制約により非対応 |
| 塗りつぶしのパターン・グラデーション | ソリッド塗りつぶし（単色）の前景色のみ対応。パターンやグラデーションは利用頻度が低く、出力が複雑になるため非対応 |
| 条件付き書式の評価 | セルの表示形式（`display`）は静的な数値フォーマットのみ適用する。条件付き書式は実行時の評価が必要であり、Excelエンジンなしでは正確に再現できない |
| 正規表現による検索 | `--query` は部分一致のみ。正規表現はシェル側の `grep` と組み合わせて実現する想定 |
| チャートシート・マクロシートのダンプ | ワークシートのみ対応。`info` コマンドではシート種類として表示するが、`dump` / `search` の対象外とする |
| Excel書き込み・編集 | 読み取り専用ツールとして設計。書き込みは別ツールの責務とする |
| 複数シートの同時指定・全シート指定 | `--sheet` は常に単一シートのみ指定可能。`--sheet all` やワイルドカード等による複数シート・全シート指定には対応しない。複数シートを処理する場合はシートごとにコマンドを実行する。`info` でシート一覧を確認し、対象シートを特定する運用を想定。これはブック横断検索（`search --all-sheets` 等）も同様で、対応しない |
| メモリ使用量の上限 | 設けない。excelizeの通常読み取りでファイル全体をメモリに展開する方式とし、メモリ制限やファイルサイズ制限は設定しない |
