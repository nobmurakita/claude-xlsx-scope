...
allowed-tools: Bash(bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope *), Read
---
...
1. `image` サブコマンドで一時ファイルに保存する: `bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope image <file> <image_id>`
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope info <file>
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope scan --sheet <name|index> <file>
```
...
- `value_count`: 値を持つセルの数。used_range が広くても value_count が少なければ図形主体のシート
- `merged_cells`: 結合セルの数。多ければ帳票・ドキュメント的なレイアウト
- `style_variants`: 視覚的に意味のあるスタイル（背景色・太字・罫線等）のバリエーション数
- `has_shapes`: 図形が存在する場合のみ `true`。`shapes` コマンドを使うべきか判断に使う

### cells

```bash
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope cells [options] <file>
```
...
```bash
# A1:H200 の最初の1000セル
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope cells --sheet 0 --range A1:H200 example.xlsx
# 最終行: {"_truncated":true,"next_cell":"A101"}
# 範囲内の続きを取得
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope cells --sheet 0 --range A1:H200 --start A101 example.xlsx
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope shapes [options] <file>
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope image example.xlsx xl/media/image1.png
```

### image

```bash
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope image <file> <image_id>
```

`shapes` 出力の `image_id`（ZIP内のメディアパス）を指定して、画像を一時ファイルに保存する。パスが stdout に出力される。

```bash
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope image example.xlsx xl/media/image1.png
# stdout: /var/folders/.../xlsx-scope-1234567.png
```

### search

```bash
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope search [options] <file>
```
...
出力例:
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/xlsx-scope search --text "合計" --sheet 0 example.xlsx
```
```jsonl
{"cell":"A10","value":"合計"}
{"cell":"A25","value":"小合計"}
