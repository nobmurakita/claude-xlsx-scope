...
allowed-tools: Bash(${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx *), Read
---
...
1. `image` サブコマンドで一時ファイルに保存する: `${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx image <file> <image_id>`
...
```bash
${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx info <file>
```
...
```bash
${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx scan --sheet <name|index> <file>
```
...
```bash
${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx cells [options] <file>
```
...
```bash
# A1:H200 の最初の1000セル
${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx cells --sheet 0 --range A1:H200 example.xlsx
# 最終行: {"_truncated":true,"next_cell":"A101"}
# 範囲内の続きを取得
${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx cells --sheet 0 --range A1:H200 --start A101 example.xlsx
```
...
```bash
${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx shapes [options] <file>
```
...
```bash
${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx image example.xlsx xl/media/image1.png
```

### image

```bash
${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx image <file> <image_id> [output]
```
...
```bash
${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx search [options] <file>
```
...
出力例:
```bash
${CLAUDE_SKILL_DIR}/scripts/cc-read-xlsx search --text "合計" --sheet 0 example.xlsx
```
```jsonl
{"cell":"A10","value":"合計"}
{"cell":"A25","value":"小合計"}
