...
1. `image` サブコマンドで一時ファイルに保存する: `bash ${CLAUDE_SKILL_DIR}/scripts/read-xlsx image <file> <image_id>`
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/read-xlsx info <file>
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/read-xlsx scan --sheet <name|index> <file>
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/read-xlsx cells [options] <file>
```
...
```bash
# A1:H200 の最初の1000セル
bash ${CLAUDE_SKILL_DIR}/scripts/read-xlsx cells --sheet 0 --range A1:H200 example.xlsx
# 最終行: {"_truncated":true,"next_cell":"A101"}
# 範囲内の続きを取得
bash ${CLAUDE_SKILL_DIR}/scripts/read-xlsx cells --sheet 0 --range A1:H200 --start A101 example.xlsx
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/read-xlsx shapes [options] <file>
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/read-xlsx image <file> <image_id>
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/read-xlsx search [options] <file>
```
...
出力例:
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/read-xlsx search --text "合計" --sheet 0 example.xlsx
```
```jsonl
{"cell":"A10","value":"合計"}
{"cell":"A25","value":"小合計"}
